...
if (is_numeric($id)) {
    ...
    if (count($data['custom_fields']) > 0) {
        foreach ($data['custom_fields'] as $field) {
            array_push($aColumns, '(SELECT value FROM ' . db_prefix() . 'fmlcustomfieldvalues LEFT JOIN ' . db_prefix() . 'fmlcustomfields ON ' . db_prefix() . 'fmlcustomfields.customfieldid = ' . $field['customfieldid'] . ' WHERE ' . db_prefix() . 'fmlcustomfieldvalues.customfieldid ="' . $field['customfieldid'] . '" AND (' . db_prefix() . 'fmlcustomfieldvalues.emailid = ' . db_prefix() . 'flexlistemails.emailid))');
        }
    }
    $sIndexColumn = 'emailid';
    $sTable       = db_prefix() . 'flexlistemails';
    ...
    foreach ($rResult as $aRow) {
        ...
        if (has_permission('flexstage', '', 'delete')) {
            $row[] = '<a href="' . admin_url('flexstage/delete_mail_list/' . $aRow['emailid']) . '"
