...
if (!$CI->db->table_exists(db_prefix() . 'ware_unit_type')) { 
    $CI->db->query('CREATE TABLE `' . db_prefix() . "ware_unit_type` (
      ...
      `display` int(1) NULL COMMENT 'display 1: display (yes)  0: not displayed (no)',
      `note` text NULL,
      `unit` varchar(50) NULL COMMENT 'Short unit name (e.g., Kg, g, L)',
      `fullName` varchar(255) NULL COMMENT 'Full unit name (e.g., Kilogram, Liter)',
      `unitQuantityCode` varchar(50) NULL COMMENT 'Standard unit code (e.g., KGS-KILOGRAMS)',
      `decimalPlaces` int(11) NULL COMMENT 'Number of decimal places used',
      `applicableFrom` DATE NULL COMMENT 'Date from which unit is applicable',
      PRIMARY KEY (`unit_type_id`)
    ) ENGINE=InnoDB DEFAULT CHARSET=" . $CI->db->char_set . ';');

    // Insert Default Units with All Parameters
    $default_units = [
        ['KG', 'Kilogram', 'Kg', 1, 1, 'Weight measurement', 'Kg', 'Kilogram', 'KGS-KILOGRAMS', 2, '2024-04-01'],
        ['G', 'Gram', 'g', 2, 1, 'Weight measurement', 'g', 'Gram', 'GRM-GRAMS', 3, '2024-04-01'],
        ['MG', 'Milligram', 'mg', 3, 1, 'Weight measurement', 'mg', 'Milligram', 'MGM-MILLIGRAMS', 3, '2024-04-01'],
        ['TON', 'Metric Ton', 'ton', 4, 1, 'Heavy weight measurement', 'ton', 'Metric Ton', 'MTN-METRIC TONS', 2, '2024-04-01'],
        ['L', 'Liter', 'L', 5, 1, 'Volume measurement', 'L', 'Liter', 'LTR-LITERS', 2, '2024-04-01'],
        ['ML', 'Milliliter', 'ml', 6, 1, 'Volume measurement', 'ml', 'Milliliter', 'MLT-MILLILITERS', 3, '2024-04-01'],
        ['M', 'Meter', 'm', 7, 1, 'Length measurement', 'm', 'Meter', 'MTR-METERS', 2, '2024-04-01'],
        ['CM', 'Centimeter', 'cm', 8, 1, 'Length measurement', 'cm', 'Centimeter', 'CM-CENTIMETERS', 2, '2024-04-01'],
        ['MM', 'Millimeter', 'mm', 9, 1, 'Length measurement', 'mm', 'Millimeter', 'MM-MILLIMETERS', 3, '2024-04-01'],
        ['SQM', 'Square Meter', 'sqm', 10, 1, 'Area measurement', 'sqm', 'Square Meter', 'SQM-SQUARE METERS', 2, '2024-04-01'],
        ['PCS', 'Pieces', 'pcs', 11, 1, 'General quantity measurement', 'pcs', 'Pieces', 'PCS-PIECES', 0, '2024-04-01'],
        ['SET', 'Set', 'set', 12, 1, 'Grouping of items', 'set', 'Set', 'SET-SETS', 0, '2024-04-01'],
        ['BOX', 'Box', 'box', 13, 1, 'Packaging unit', 'box', 'Box', 'BOX-BOXES', 0, '2024-04-01'],
        ['PAIR', 'Pair', 'pair', 14, 1, 'Two items together', 'pair', 'Pair', 'PAIR-PAIRS', 0, '2024-04-01'],
        ['DOZ', 'Dozen', 'doz', 15, 1, 'Twelve items', 'doz', 'Dozen', 'DOZ-DOZENS', 0, '2024-04-01'],
        ['HR', 'Hour', 'hr', 16, 1, 'Time unit', 'hr', 'Hour', 'HR-HOURS', 2, '2024-04-01'],
        ['DAY', 'Day', 'day', 17, 1, 'Time unit', 'day', 'Day', 'DAY-DAYS', 0, '2024-04-01'],
        ['WEEK', 'Week', 'week', 18, 1, 'Time unit', 'week', 'Week', 'WEEK-WEEKS', 0, '2024-04-01'],
        ['MONTH', 'Month', 'month', 19, 1, 'Time unit', 'month', 'Month', 'MNTH-MONTHS', 0, '2024-04-01']
    ];

    foreach ($default_units as $unit) {
        $CI->db->query("INSERT INTO `" . db_prefix() . "ware_unit_type`
        (`unit_code`, `unit_name`, `unit_symbol`, `order`, `display`, `note`, `unit`, `fullName`, `unitQuantityCode`, `decimalPlaces`, `applicableFrom`)
        VALUES ('{$unit[0]}', '{$unit[1]}', '{$unit[2]}', {$unit[3]}, {$unit[4]}, '{$unit[5]}', '{$unit[6]}', '{$unit[7]}', '{$unit[8]}', {$unit[9]}, '{$unit[10]}')");
    }
}
