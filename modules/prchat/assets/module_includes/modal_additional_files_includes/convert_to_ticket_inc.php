     ...
     $('body').on('click', '#convertToTicket', function() {
          ...
          if (subject !== '') {
               $.post('createNewSupportTicket', {
                         ...
                         $.post("<?= site_url('prchat/Prchat_ClientsController/trigger_ticket_event'); ?>", {
                              ...
                              if (r.result == 'success') {
                                   setTimeout(function() {
                                        ...
                                        $('.client_chatbox').html('<?= _l('chat_client_created_new_support_ticket'); ?><a class="chat_ticket_link" href="' + location.origin + '/clients/ticket/' + r.ticket_id + '" target="_blank"><?= _l('support_ticket'); ?></a>');
                    ...
                    // alert_float('danger', 'Something went wrong... try again ' + error.statusText);
