(function(){
  ...
  initDataTable('.table-channel-flipkart', admin_url + 'omni_sales/table_channel_flipkart', false, false, fnServerParams, [0, 'desc']);
  initDataTable('.table-product-flipkart', admin_url + 'omni_sales/table_product_flipkart', '', '', fnServerParams);
  initDataTable('.table-channel-myntra', admin_url + 'omni_sales/table_channel_myntra', false, false, fnServerParams, [0, 'desc']);
  initDataTable('.table-product-myntra', admin_url + 'omni_sales/table_product_myntra', '', '', fnServerParams);
  initDataTable('.table-channel-amazon', admin_url + 'omni_sales/table_channel_amazon', false, false, fnServerParams, [0, 'desc']);
  initDataTable('.table-product-amazon', admin_url + 'omni_sales/table_product_amazon', '', '', fnServerParams);
  ...
  $('#add_channel_flipkart').click(function(){
    $('.add-title').removeClass('hide');
    $('.update-title').addClass('hide');
    $('.test_connect').addClass('hide');
    $('#channel_flipkart').modal('show');
    $('input[name="id"]').val('');
    $('input[name="name_channel"]').val('');
    $('input[name="consumer_key"]').val('')
    $('input[name="consumer_secret"]').val('');
  ...
  $('#add_channel_myntra').click(function(){
    $('.add-title').removeClass('hide');
    $('.update-title').addClass('hide');
    $('.test_connect').addClass('hide');
    $('#channel_myntra').modal('show');
    $('input[name="id"]').val('');
    $('input[name="name_channel"]').val('');
    $('input[name="consumer_key"]').val('')
    $('input[name="consumer_secret"]').val('');
  ...
  $('#add_channel_amazon').click(function(){
    $('.add-title').removeClass('hide');
    $('.update-title').addClass('hide');
    $('.test_connect').addClass('hide');
    $('#channel_amazon').modal('show');
    $('input[name="id"]').val('');
    $('input[name="name_channel"]').val('');
    $('input[name="consumer_key"]').val('')
    $('input[name="consumer_secret"]').val('');
  ...
  $('.test_connect_myntra').click(function(){
    var url = $('input[name="url"]').val();
    var consumer_key = $('input[name="consumer_key"]').val();
    var consumer_secret = $('input[name="consumer_secret"]').val();
    var refresh_token = $('input[name="refresh_token"]').val();
    var html = '';
    html += '<div class="Box">';
    html += '<span>';
    html += '<span></span>';
    ...
    $.post(admin_url+'omni_sales/test_connect_myntra', data).done(function(response){
      response = JSON.parse(response);
      if(response.check == true){
        alert_float('success', response.message);
      }else{
  ...
  $('.test_connect_amazon').click(function(){
    var url = $('input[name="url"]').val();
    var consumer_key = $('input[name="consumer_key"]').val();
    var consumer_secret = $('input[name="consumer_secret"]').val();
    var refresh_token = $('input[name="refresh_token"]').val();
    ...
    $.post(admin_url+'omni_sales/test_connect_amazon', data).done(function(response){
      response = JSON.parse(response);
      if(response.check == true){
        alert_float('success', response.message);
      }else{
...
function edit_myntra(el){
  ...
  var secret= $(el).data("secret");
  var url= $(el).data("url");
  var refresh_token= $(el).data("refresh-token");
  
  $('.update-title').removeClass('hide');
  $('.add-title').addClass('hide');
  $('.test_connect_myntra').removeClass('hide');
  $('input[name="id"]').val(id);
  $('input[name="name_channel"]').val(name);
  $('input[name="consumer_key"]').val(key)
  $('input[name="consumer_secret"]').val(secret);
...
function edit_amazon(el){
  ...
  $('.update-title').removeClass('hide');
  $('.add-title').addClass('hide');
  $('.test_connect_amazon').removeClass('hide');
  $('input[name="id"]').val(id);
  $('input[name="name_channel"]').val(name);
  $('input[name="consumer_key"]').val(key)
  $('input[name="consumer_secret"]').val(secret);
...
function sync_store_flipkart(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  html += '</div>';
  $('#box-loadding').html(html);
  $.post(admin_url+'omni_sales/process_orders_flipkart/'+id).done(function(response){
    $('#box-loadding').html('');
    $('.status-sync').removeClass('label-danger');
    $('.status-sync').addClass('label-success');
    $('.status-sync').text('Sync success');
...
function sync_store_myntra(el){
  ...
  html += '</div>';
  $('#box-loadding').html(html);
  $.post(admin_url+'omni_sales/process_orders_myntra/'+id).done(function(response){
    $('#box-loadding').html('');
    $('.status-sync').removeClass('label-danger');
    $('.status-sync').addClass('label-success');
    $('.status-sync').text('Sync success');
...
function sync_store_amazon(el){
  ...
  html += '</div>';
  $('#box-loadding').html(html);
  $.post(admin_url+'omni_sales/process_orders_amazon/'+id).done(function(response){
    $('#box-loadding').html('');
    $('.status-sync').removeClass('label-danger');
    $('.status-sync').addClass('label-success');
    $('.status-sync').text('Sync success');
...
function sync_inventory_synchronization_fkt(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_asynclibrary_inventory_detail_flipkart/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_inventory_synchronization_myntra(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_asynclibrary_inventory_detail_myntra/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_inventory_synchronization_amazon(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_asynclibrary_inventory_detail_amazon/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_decriptions_synchronization_fkt(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_decriptions_synchronization_detail_flipkart/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_decriptions_synchronization_myntra(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_decriptions_synchronization_detail_myntra/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_decriptions_synchronization_amazon(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_decriptions_synchronization_detail_amazon/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_images_synchronization_fkt(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_asynclibrary_image_detail_flipkart/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_images_synchronization_myntra(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_asynclibrary_image_detail_myntra/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_images_synchronization_amazon(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      html += '</div>';
      $('#box-loadding').html(html);
      $.post(admin_url+'omni_sales/process_asynclibrary_image_detail_amazon/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function omi_sales_delete_bulk_action_flipkart(event) {
  "use strict";
    var store = $("input[name='id']").val();
    var arr_id = $("input[name='check']").val();
    if (confirm_delete()) {
        ...
        if(mass_delete == true){
            ...
            data.store = store;
            data.arr_id = arr_id;
            var rows = $('#table-product-flipkart').find('tbody tr');
            $.each(rows, function() {
                var checkbox = $($(this).find('td').eq(0)).find('input');
                if (checkbox.prop('checked') === true) {
                    ids.push(checkbox.val());
            ...
            $(event).addClass('disabled');
            setTimeout(function() {
                $.post(admin_url + 'omni_sales/omi_sales_delete_bulk_action_flipkart', data).done(function() {
                    window.location.reload();
                }).fail(function(data) {
                    $('#product-woocommerce_bulk_actions').modal('hide');
                    alert_float('danger', data.responseText);
...
function omi_sales_delete_bulk_action_myntra(event) {
  "use strict";
    var store = $("input[name='id']").val();
    var arr_id = $("input[name='check']").val();
    if (confirm_delete()) {
        ...
        if(mass_delete == true){
            ...
            data.store = store;
            data.arr_id = arr_id;
            var rows = $('#table-product-myntra').find('tbody tr');
            $.each(rows, function() {
                var checkbox = $($(this).find('td').eq(0)).find('input');
                if (checkbox.prop('checked') === true) {
                    ids.push(checkbox.val());
            ...
            $(event).addClass('disabled');
            setTimeout(function() {
                $.post(admin_url + 'omni_sales/omi_sales_delete_bulk_action_myntra', data).done(function() {
                    window.location.reload();
                }).fail(function(data) {
                    $('#product-woocommerce_bulk_actions').modal('hide');
                    alert_float('danger', data.responseText);
...
function omi_sales_delete_bulk_action_amazon(event) {
  "use strict";
    var store = $("input[name='id']").val();
    var arr_id = $("input[name='check']").val();
    if (confirm_delete()) {
        ...
        if(mass_delete == true){
            ...
            data.store = store;
            data.arr_id = arr_id;
            var rows = $('#table-product-amazon').find('tbody tr');
            $.each(rows, function() {
                var checkbox = $($(this).find('td').eq(0)).find('input');
                if (checkbox.prop('checked') === true) {
                    ids.push(checkbox.val());
            ...
            $(event).addClass('disabled');
            setTimeout(function() {
                $.post(admin_url + 'omni_sales/omi_sales_delete_bulk_action_amazon', data).done(function() {
                    window.location.reload();
                }).fail(function(data) {
                    $('#product-woocommerce_bulk_actions').modal('hide');
                    alert_float('danger', data.responseText);
...
function sync_price_flipkart(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_price_flipkart/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_price_myntra(el){
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_price_myntra/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_price_amazon(el){
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_price_amazon/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_all_fkt(el){
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_all_flipkart/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        response = JSON.parse(response);
...
function sync_all_myntra(el){
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_all_myntra/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        response = JSON.parse(response);
...
function sync_all_amazon(el){
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_all_amazon/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        response = JSON.parse(response);
...
function sync_product_name_fkt(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_product_name_flipkart/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_product_name_myntra(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_product_name_myntra/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_product_name_amazon(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_product_name_amazon/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_short_description_flipkart(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_short_description_flipkart/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_short_description_myntra(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_short_description_myntra/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function sync_short_description_amazon(el){
  "use strict";
  $('.status-sync').removeClass('label-primary');
  $('.status-sync').addClass('label-danger');
  $('.status-sync').text('Wait for sync');
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/sync_short_description_amazon/', data).done(function(response){
       $('.status-sync').removeClass('label-danger');
        $('.status-sync').addClass('label-success');
        $('.status-sync').text('Sync success');
        $('#box-loadding').html('');
...
function price_update_fkt(el){
  ...
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/fkt_price_update/', data).done(function(response){
        response = JSON.parse(response);
        if(response){
          $('#box-loadding').html('');
          alert_float('success', 'Update successfully');
          $('.table-product-flipkart').DataTable().ajax.reload();
        }else{
          $('#box-loadding').html('');
          alert_float('warning', 'Update unsuccessful');
        }
...
function price_update_myntra(el){
  var id = $(el).data('id');
  var check_detail =  $("input[name='check']").val();
  var arr_val = check_detail.split(',');
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/myntra_price_update/', data).done(function(response){
        response = JSON.parse(response);
        if(response){
          $('#box-loadding').html('');
          alert_float('success', 'Update successfully');
...
function price_update_amazon(el){
  var id = $(el).data('id');
  var check_detail =  $("input[name='check']").val();
  var arr_val = check_detail.split(',');
  if(arr_val.length > 0 && arr_val[0] != ""){
      ...
      $.post(admin_url+'omni_sales/amazon_price_update/', data).done(function(response){
        response = JSON.parse(response);
        if(response){
          $('#box-loadding').html('');
          alert_float('success', 'Update successfully');
