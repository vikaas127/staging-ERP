...
$where = [];
$join[] = 'LEFT JOIN '.db_prefix().'clients ON '.db_prefix().'mrp_manufacturing_orders.contact_id = '.db_prefix().'clients.userid';
    ...
    foreach ($customer_filter as $contact_id) {
        if ($contact_id != '') {
            if ($where_customer_ft == '') {
                $where_customer_ft .= 'AND (' . db_prefix() . 'mrp_manufacturing_orders.contact_id = "' . $contact_id . '"';
            } else {
                $where_customer_ft .= ' OR ' . db_prefix() . 'mrp_manufacturing_orders.contact_id = "' . $contact_id . '"';
            }
        }
    }
    if ($where_customer_ft != '') {
...
foreach ($rResult as $aRow) {
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		}elseif ($aColumns[$i] == 'contact_id') {  // <-- Add your condition right here
