...
foreach ($rResult as $aRow) {
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == 'id') {
			...
			$code .= '<a href="' . admin_url('manufacturing/view_routing/' . $aRow['id']) . '" >' . _l('view') . '</a>';

			if (has_permission('manufacturing', '', 'edit')&&has_permission('routing', '', 'edit') || is_admin()) {

				$code .= ' | <a href="' . admin_url('manufacturing/operation_manage/' . $aRow['id']) . '" >' . _l('edit') . '</a>';
			}
			if (has_permission('manufacturing', '', 'delete')&&has_permission('routing', '', 'delete') || is_admin()) {
