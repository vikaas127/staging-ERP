...
foreach ($rResult as $aRow) {
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == 'id') {
			...
			if (has_permission('manufacturing', '', 'edit')&&has_permission('work_centers', '', 'edit') || is_admin()) {

				$code .= ' | <a href="' . admin_url('manufacturing/add_edit_work_center/' . $aRow['id']) . '" >' . _l('edit') . '</a>';
			}
			if (has_permission('manufacturing', '', 'delete')&&has_permission('work_centers', '', 'delete') || is_admin()) {
