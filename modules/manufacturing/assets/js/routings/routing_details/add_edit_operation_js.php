<script>
	...
	var workCentersData = <?php echo json_encode($work_centers); ?>;

function updateSubcontractorDisplay() {
  const selectedWorkCenterId = $('#work_center_id').val();
  const selectedWorkCenter = workCentersData.find(wc => wc.id == selectedWorkCenterId);

  if (selectedWorkCenter && selectedWorkCenter.is_subcontract == 1) {
    $('#vendor_display_section').show();
    $('#subcontractor_display').val(selectedWorkCenter.vendor_name || 'N/A');
  } else {
    $('#vendor_display_section').hide();
    $('#subcontractor_display').val('');
  }
}

$(document).ready(function() {
  updateSubcontractorDisplay(); // run on load (edit mode)
  $('#work_center_id').on('change', updateSubcontractorDisplay);
});
