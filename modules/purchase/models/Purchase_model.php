...
{   
    ...
    {   
        ...
        $this->db->where('option_name', 'estimate_prefix');
        $this->db->update(db_prefix() . 'purchase_option', [
            'option_val' => $data['estimate_prefix'],
        ]);
        if ($this->db->affected_rows() > 0) {
            $rs++;
        } else {
            log_message('debug', 'estimate_prefix NOT updated. Tried value: ' . print_r($data['estimate_prefix'], true));
        }
...
public function generate_commodity_code() {
    ...
    // Find the last inserted commodity_code starting with PUR-
    $this->db->like('commodity_code', $prefix);
    $this->db->order_by('id', 'DESC');
    $last_item = $this->db->get(db_prefix().'items')->row();

    if ($last_item && isset($last_item->commodity_code)) {
        $last_number = (int) str_replace($prefix, '', $last_item->commodity_code);
