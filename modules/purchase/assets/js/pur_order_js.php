...
function estimate_by_vendor(invoker){
  "use strict";
    console.log("estimate_by_vendor called, vendor ID:", invoker.value);
  ...
  if(invoker.value != 0){
    $.post(admin_url + 'purchase/estimate_by_vendor/'+invoker.value).done(function(response){
      response = JSON.parse(response);
      console.log("Item List (option_html):", response.option_html);
      ...
      <?php } ?>
      console.log("item_by_vendor setting: <?php echo get_purchase_option('item_by_vendor'); ?>");

      <?php if(get_purchase_option('item_by_vendor') == 1){ ?>
        console.log("response.option_html value:", response.option_html);
