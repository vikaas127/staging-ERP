...
/**
    ...
    $commodity_code = $this->purchase_model->generate_commodity_code(); 
    echo json_encode($commodity_code);
    die;
}
...
public function get_item_by_id($id, $currency_rate = 1)
{
    ...
    if ($this->input->is_ajax_request()) {
        ...
        log_message('info', 'AJAX request received for get_item_by_id. ID: ' . $id . ', Currency Rate: ' . $currency_rate);

        // Fetch item
        $item = $this->purchase_model->get_item_v2($id);

        if (!$item) {
            log_message('error', 'Item not found for ID: ' . $id);
            echo json_encode(['error' => 'Item not found']);
            return;
        }

        // Convert long description line breaks
        $item->long_description = nl2br($item->long_description ?? '');

        log_message('debug', 'Item retrieved: ' . print_r($item, true));

        // Apply currency rate conversion if needed
        if ($currency_rate != 1) {
            $original_price = $item->purchase_price;
            $item->purchase_price = round(($item->purchase_price * $currency_rate), 2);
            log_message('info', 'Currency rate applied. Original price: ' . $original_price . ', Converted price: ' . $item->purchase_price);
        }

        // Initialize warehouses_html
        $html = '<option value=""></option>';
        $item->warehouses_html = $html;

        // Log final item object before sending
        log_message('debug', 'Final item response: ' . print_r($item, true));

        // Return JSON
        echo json_encode($item);
    } else {
        log_message('error', 'Non-AJAX request attempted for get_item_by_id.');
        show_404();
    }
}
