...
class Invoices extends AdminController
{
    ...
    public function saveInvoice($invoiceData) {
        // Assuming you have a table 'tblinvoices' to save the invoice data
    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                // Add E-Invoice number if provided
                if (isset($invoice_data['einvoice_number']) && !empty($invoice_data['einvoice_number'])) {
                    $invoice_data['einvoice_number'] = trim($invoice_data['einvoice_number']);
                }
    ...
    public function getAuthToken($gstin) {
        // Search in `tbleway_bill`
        $query = $this->db->get_where('tbleway_bill', ['gstin' => $gstin], 1);
        if ($query->num_rows() > 0) {
            return $query->row_array();
        }

        // Search in `tble_invoice` if not found
        $query = $this->db->get_where('tble_invoice', ['gstin' => $gstin], 1);
        if ($query->num_rows() > 0) {
            return $query->row_array();
        }
