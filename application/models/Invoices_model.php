...
class Invoices_model extends App_Model
{
    ...
    {
        $this->load->model('Invoices_model'); // Load the model

        $invoices = $this->Invoices_model->get_all_invoices(); // Fetch all invoices

        if ($invoices) {
            echo json_encode(['status' => 'success', 'data' => $invoices]);
        } else {
            echo json_encode(['status' => 'error', 'message' => 'No invoices found or table does not exist.']);
        }
    }
    ...
    {
        $query = $this->db->get('tbl_invoice'); // Get all records from the table
    ...
    {
        $data['e_invoice_number'] = 'E' . rand(100000, 999999);
        $data['irn'] = 'IRN' . rand(1000000, 9999999);
        $data['qr_code'] = "QRCode_" . time();
        $data['einvoice_status'] = 'Pending';
        ...
        } elseif (isset($data['save_and_generate_einvoice'])) {
            $data['status'] = self::STATUS_DRAFT;
            $data['is_e_invoice'] = 1; // Mark as E-Invoice
        }


        // Add condition for save_and_generate_einvoice
        // if (isset($data['save_and_generate_einvoice'])) {
        //     $data['status'] = self::STATUS_DRAFT; // Or set another status if required
        //     $data['is_e_invoice'] = 1; // Mark as E-Invoice
        //     $data['e_invoice_number'] = 'E' . rand(100000, 999999);; // You will need to create a function to generate this
        //     $data['irn'] = 'IRN' . rand(1000000, 9999999); // Generate IRN as needed
        //     $data['qr_code'] =  'QRCodeFor_' . $data['e_invoice_   number'] . '_' . $data['irn'];// Generate QR code based on e-invoice and IRN
        //     $data['einvoice_status'] = 'Pending'; // Default to Pending
        //     $data['eway_bill_number'] = 'EB' . rand(1000000, 9999999); // Generate E-Way Bill Number as needed
        //     unset($data['save_and_generate_einvoice']);
        // }
        ...
        if ($insert_id) {
            ...
            foreach ($invoices_to_merge as $m) {
                ...
                if ($data['is_e_invoice'] == 1) {
                    // Insert data into 'tblinvoices' table
                    $insert = $this->db->insert('tblinvoices', $data);

                    // Check if the insert was successful
                    if ($insert) {
                        echo json_encode([
                            "success" => true,
                            "message" => "E-Invoice data inserted successfully!"
                        ]);
                    } else {
                        echo json_encode([
                            "success" => false,
                            "message" => "Error inserting E-Invoice data: " . $this->db->error()
                        ]);
                    }
                }

                // if ($data['is_e_invoice'] == 1) {
                //     // Call method to generate e-invoice (or trigger API call to generate e-invoice if required)
    ...
    public function insertInvoice($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_message('info', '✅ Insert Successful: ' . json_encode($data)); // ✅ Log successful insert
            return $this->db->insert_id();
        } else {
            log_message('error', '❌ Database Insert Failed: ' . json_encode($data)); // ✅ Log failure
            return false;
        }
    }
    // Function to generate e-invoice number (you may need to customize the logic)
