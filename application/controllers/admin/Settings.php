...
class Settings extends AdminController
{
    ...
    public function get_company_info()
    {
        $data = [
            ...
            'phone_number' => get_option('invoice_company_phonenumber'),
            'vat_number' => get_option('company_vat'),
            'company_info_format' => clear_textarea_breaks(get_option('company_info_format')),
        ];
        ...
        if (!empty($custom_fields)) {
            foreach ($custom_fields as $field) {
                $data['custom_fields'][$field['name']] = '{cf_' . $field['id'] . '}';
            }
        }
