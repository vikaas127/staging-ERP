...
class Authentication extends ClientsController
{
    ...
    public function register()
    {
        ...
        $fields = [
            ...
            'sector'    => $honeypot ? 'sectormjxw' : 'sector',
            'industry'  => $honeypot ? 'industrymjxw' : 'industry',
        ...
        $this->form_validation->set_rules($fields['sector'], _l('client_sector'), 'required');
        $this->form_validation->set_rules($fields['industry'], _l('client_industry'), 'required');
        ...
        if ($this->input->post()) {
            if ($honeypot &&
            count(array_filter($this->input->post(['email', 'firstname', 'lastname', 'company', 'sector', 'industry']))) > 0) {
             show_404();
            }
